...
if __name__ == '__main__':
    ...
    try:
        ...
        #testReturn2 = wsF.checkParamExistsOnServer(connection,'/test/int')
        #print("finished attempting to open connection")
        #print("testReturn2 = %s" % str(testReturn2))
        #sleep(3)
        print("Force wait until rosbridge interface and parameter server comes online...")
        testReturn = wsF.generalRecvParamServer(connection,'/',['rosdistro'],'block',1.0) # wait 1.0sec before recheck
        print("Parameter server is up and responding!")
        #print("testReturn = %s" % str(testReturn))
        #sleep(3)
        #print("Force wait until rosbridge interface and parameter server AND MOBILESIM comes online...")
        #testReturn3 = wsF.generalRecvTopicsServicesList(connection,"/rosapi/topics",None,'block')
        #print("Parameter server is up and responding!")
        #print("testReturn3 = %s" % str(testReturn3))
        #print("Done-good. yay?")
